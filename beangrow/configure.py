...
def find_accounts(
    ...
    return sorted(
        ...
        if (
            accountlib.leaf(account) in commodities
            and acctypes.is_balance_sheet_account(account, atypes)
            and not acctypes.is_equity_account(account, atypes)
            and (_close is None or (start_date and _close.date > start_date))
        )
    )
...
def infer_investments_configuration(
    ...
    for account in account_list:
        ...
        for entry in data.filter_txns(entries):
            if any(
                posting.account in match_accounts for posting in entry.postings
            ):
                for posting in entry.postings:
                    if (
                        posting.account == aconfig.asset_account
                    ...
                    if (
                        re.search(
                            r':(Cash|Checking|Receivable|GSURefund)$',
                            posting.account,
                        )
                        or re.search(r'Receivable|Payable', posting.account)
                        or re.match(r'Income:.*:(Match401k)$', posting.account)
                    ):
                        cash_accounts.add(posting.account)
        aconfig.cash_accounts.extend(cash_accounts)


def infer_report_groups(
    entries: data.Entries,
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            'Accounts already closed before this date will not be '
