...
def main():
    ...
    with beam.Pipeline(options=poptions) as pipeline:
        ...
        for report in config.groups.group:
            ...
            assert all(
                isinstance(ad, investments.AccountData) for ad in adlist
            )
...
def WriteNamedBinary(
    item: Tuple[str, bytes], filename_fn: Callable[[str], str]
):
...
def main_saved_for_later():
    ...
    reports.generate_price_pages(
        account_data_map, prices.build_price_map(entries), output_prices
    )

    # Output required price directives (to be filled in the source ledger by
    # fetching prices).
    reports.write_price_directives(
        path.join(output_prices, 'prices.beancount'),
        pricer,
