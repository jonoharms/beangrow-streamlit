...
def read_config(
    config_filename: str, filter_reports: list[str], accounts: list[Account]
) -> Config:
...
def post_process_config(
    config: Config, filter_reports: list[str], accounts: list[Account]
) -> Config:
    ...
    for investment in config.investments.investment:
        ...
        investment.dividend_accounts[:] = expand_globs(
            investment.dividend_accounts, accounts
        )
    ...
    if filter_reports:
        reports = [
            ...
            for report in config.groups.group
            if any(
                fnmatch.fnmatch(report.name, pattern)
    ...
    used_investments = set(
        inv for report in config.groups.group for inv in report.investment
    )
    investments = [
        invest
